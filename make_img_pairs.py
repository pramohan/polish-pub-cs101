...
def convolvehr(
    data, kernel, plotit=False, rebin=4, norm=True, nbit=16, noise=True, cmap="afmhot"
):
    ...
    if plotit:
        ...
        plt.hist(dataLRflat, color="C1", alpha=0.5, density=True, log=True, bins=255)
        plt.hist(dataflat, bins=255, color="C0", alpha=0.25, density=True, log=True)
        ...
        if norm is False:
            data = (
                data.reshape(data.shape[0] // 4, 4, data.shape[-2] // 4, 4, ncolor)
                .mean(1)
                .mean(-2)
            )
            plt.imshow(dataLR[..., 0], cmap=cmap, vmax=dataLR[..., 0].max() * 0.025)
        else:
...
def create_LR_image(
    ...
    for ii in range(nimages):
        ...
        if galaxies:
            ...
            nsrc = np.random.poisson(
                int(src_density * (Nx * Ny * PIXEL_SIZE**2 / 60.0**2))
            )
            fdirgalparams = fdirout + "/galparams/"
            if not os.path.isdir(fdirgalparams):
                os.system("mkdir %s" % fdirgalparams)
            fnblobout = (
                fdirgalparams + fn.split("/")[-1].strip(".png") + "GalParams.txt"
            )
        ...
        if distort_psf:
            for aa in [1]:
                ...
                kernel_ = elastic_transform(
                    kernel_, alpha=alphad, sigma=3, alpha_affine=0
                )
                if plotit:
                    plt.subplot(132)
                    ...
                    plt.subplot(133)
                    plt.imshow(
                        kernel - kernel_[..., 0], vmax=0.1, vmin=-0.1, cmap="Greys"
                    )
                    plt.colorbar()
                    plt.show()
                ...
                np.save(
                    fdiroutPSF + fn.split("/")[-1][:-4] + "-%0.2f-.npy" % alphad,
                    kernel_,
                )
        else:
            kernel_ = kernel

        dataLR, data_noise = convolvehr(
            data, kernel_, plotit=plotit, rebin=rebin, norm=norm, nbit=nbit, noise=True
        )
...
if __name__ == "__main__":
    ...
    parser.add_option(
        "-k", "--kernel", dest="kernel", type="str", help="", default="Gaussian"
    )
    parser.add_option(
        "-s", "--ksize", dest="ksize", type=int, help="size of kernel", default=256
    )
    parser.add_option(
        "-o", "--fdout", dest="fdout", type="str", help="output directory", default="./"
    )
    ...
    parser.add_option(
        "--sky", dest="sky", action="store_true", help="use SKA mid image as input"
    )
    parser.add_option(
        "-r",
        "--rebin",
    ...
    parser.add_option(
        ...
        dest="nchan",
        type=int,
        help="number of frequency channels for image",
        default=1,
    )
    parser.add_option(
        "--ntrain",
    ...
    parser.add_option(
        "--nside", dest="nside", type=int, default=2048, help="dimension of HR image"
    )
    ...
    if options.kernel.endswith("npy"):
        ...
        if abs((1 - pixel_scale_psf / PIXEL_SIZE)) > 0.025:
            print(
                "Stretching PSF by %0.3f to match map" % (pixel_scale_psf / PIXEL_SIZE)
            )
