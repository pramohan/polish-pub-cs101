...
def plot_simulated_sky():
    galsize = np.random.gamma(2.25, 1.0, 10000) * 0.5
    ellipticity = np.random.gamma(2, 3, 10000) / 20.0 * 0.5
    ellipticity = np.random.gamma(10, 10, 10000) / 1000.0
    ellipticity = np.random.beta(1.7, 4.5, 10000)
    spec_ind = np.random.normal(-0.55, 0.15, 10000)

    nfluxhigh = np.random.uniform(0, 0.1, 100000) ** (-2.0 / 3.0)
    nfluxlow = np.random.uniform(0.05, 1, 100000) ** (-1.0)
...
def plot_array(fncfg, fnpsf1, fnpsf2):
    ...
    dy /= 3e2 / 1350.0 * 1000.0  # change units to kilo lambdas
    dx /= 3e2 / 1350.0 * 1000.0  # change units to kilo lambdas
...
def plotdeconv(cmap="Greys", vmaxlr=10000, vminlr=1000, vmaxsr=5000, vminsr=0):
    ...
    plt.imshow(
        ...
        vmin=vminlr,
        aspect="auto",
        extent=[0, datalr.shape[0] * 2.0, datalr.shape[0] * 2.0, 0],
    )
    ...
    plt.imshow(
        ...
        vmin=vminsr,
        aspect="auto",
        extent=[0, datasr.shape[0] * 0.5, datasr.shape[0] * 0.5, 0],
    )
...
def plot_radial_psf(fnlr, fnhr, fnpsf, fn_model):
    ...
    ax4.text(
        0.7,
        0.1,
        r"PSF$_{true}$=1.05$\times$PSF$_{train}$",
    ...
    ax5.text(
        0.7,
        0.1,
        r"PSF$_{true}$=1.25$\times$PSF$_{train}$",
    ...
    ax6.text(
        0.7,
        0.1,
        r"PSF$_{true}$=2$\times$PSF$_{train}$",
...
def plot_example_sr(
    ...
    props = dict(facecolor="k", alpha=0.0, edgecolor="k")
    ...
    if calcpsnr:
        psnr_2 = tf.image.psnr(
            datasr_1[None, ..., None].astype(np.uint16),
            datahr_1[None, ..., None].astype(np.uint16),
            max_val=2 ** (nbit) - 1,
        )
        ssim_2 = tf.image.ssim(
            datasr_1[None, ..., None].astype(np.uint16),
            datahr_1[None, ..., None].astype(np.uint16),
            2 ** (nbit) - 1,
            filter_size=2,
        ...
        psnr_1 = tf.image.psnr(
            datasr_2[None, ..., None].astype(np.uint16),
            datahr_2[None, ..., None].astype(np.uint16),
            max_val=2 ** (nbit) - 1,
        )
        ssim_1 = tf.image.ssim(
            datasr_2[None, ..., None].astype(np.uint16),
            datahr_2[None, ..., None].astype(np.uint16),
            2 ** (nbit) - 1,
            filter_size=2,
        ...
        if nsub == 4:
            ...
            psnr_clean_1 = tf.image.psnr(
                dataclean_2[None, ..., None].astype(np.uint16),
                datahr_1[None, ..., None].astype(np.uint16),
                max_val=2 ** (nbit) - 1,
            )
            ssim_clean_1 = tf.image.ssim(
                dataclean_2[None, ..., None].astype(np.uint16),
                datahr_2[None, ..., None].astype(np.uint16),
                2 ** (nbit) - 1,
                filter_size=2,
            ...
            psnr_clean_2 = tf.image.psnr(
                dataclean_1[None, ..., None].astype(np.uint16),
                datahr_1[None, ..., None].astype(np.uint16),
                max_val=2 ** (nbit) - 1,
            )
            ssim_clean_2 = tf.image.ssim(
                dataclean_1[None, ..., None].astype(np.uint16),
                datahr_1[None, ..., None].astype(np.uint16),
                2 ** (nbit) - 1,
                filter_size=2,
    ...
    xlim1, ylim1 = (
        0.0,
        0,
    )  # 0.58#0.46029491318597404, 0.5403755860037598

    dx, dy = 1.0, 1.0  # 0.12, 0.12, #0.16, 0.16
    b, a, d, c = 230, 330, 230 + 180, 330 + 180
    a1, b1, c1, d1 = 0.12, 0.72, 0.233, 0.833
    ...
    ax1.add_patch(
        plt.Rectangle(
            (a1, b1), abs(b1 - d1), abs(c1 - a1), ls="--", lw=3, ec="C3", fc="None"
        )
    )
    ...
    ax2.add_patch(
        plt.Rectangle(
            (a1, b1), abs(b1 - d1), abs(c1 - a1), ls="--", lw=3, ec="C2", fc="None"
        )
    )
    ...
    inset_ax.imshow(
        datasr_2[a:c, b:d], cmap=cmap, vmax=vmaxsr_2, vmin=vminsr_2, extent=[0, 1, 0, 1]
    )
    inset_ax.set_yticks([])
    inset_ax.set_xticks([])
    ...
    ax5.add_patch(
        plt.Rectangle(
            (a1, b1), abs(b1 - d1), abs(c1 - a1), ls="--", lw=3, ec="grey", fc="None"
        )
    )
    ...
    inset_ax.imshow(
        datahr_1[a:c, b:d], cmap=cmap, vmax=vmaxhr_1, vmin=vminhr_1, extent=[0, 1, 0, 1]
    )
    inset_ax.set_yticks([])
    inset_ax.set_xticks([])
    ...
    inset_ax.imshow(
        datahr_2[a:c, b:d], cmap=cmap, vmax=vmaxlr_1, vmin=vminlr_1, extent=[0, 1, 0, 1]
    )
    inset_ax.set_yticks([])
    inset_ax.set_xticks([])
    inset_ax.add_patch(plt.Rectangle((0, 0), 1, 1, ls="--", lw=7, ec="grey", fc="None"))
    ...
    if nsub == 4:
        ...
        ax7.add_patch(
            plt.Rectangle(
                (a1, b1), abs(b1 - d1), abs(c1 - a1), ls="--", lw=3, ec="C0", fc="None"
            )
        )
        ...
        inset_ax.add_patch(
            plt.Rectangle((0, 0), 1, 1, ls="--", lw=7, ec="C0", fc="None")
        )
...
def save_same_data():
    ...
    srfull = srfull[
        1000 - 1875 // 2 : 1000 + 1875 // 2 + 1, 1000 - 1875 // 2 : 1000 + 1875 // 2 + 1
    ]
    sr1chan = sr1chan[
        1000 - 1875 // 2 : 1000 + 1875 // 2 + 1, 1000 - 1875 // 2 : 1000 + 1875 // 2 + 1
    ]
...
def perturbation_figure(hr, psf, model):
    ...
    rad_stretch = np.linspace(1.0, 2.0, 4)
    psnr_arr, ssim_arr = [], []
    for rr in rad_stretch:
        ...
        psnr_ = tf.image.psnr(
            dsr[None, ..., 0, None].astype(np.uint16),
...
def psf_perturbation_plot():
    ...
    rad_stretch = np.linspace(1.0, 1.35, 3)
    ...
    for ii, rr in enumerate(rad_stretch):
        for jj, nn in enumerate(nuggets):
            ...
            psf_ = elastic_transform(
                psf_[:, :, None] * np.ones([1, 1, 3]), alpha=nn, sigma=3, alpha_affine=0
            )[:, :, 0]

            n = psf_.shape[0] // 2
            plt.subplot(gs[ii, jj])
            plt.imshow(
                np.log(np.abs(psf_[n - 28 : n + 28, n - 28 : n + 28])),
                vmax=0,
                vmin=-5.5,
            ...
            plt.imshow(
                dsr[200:400, 400:600, 0], vmax=hr.max() * 0.01, cmap="afmhot_10us"
            )
            plt.axis("off")
...
def restore_CLEAN(
    fnfits_model, bmaj=3.56, bmin=3.51818, pa=-15.0404, pixel_scale=0.5, fnout=None
):
    """
...
def lobe_gal_plot(
    ...
    figure()
    subplot(131)
    imshow(lr, cmap="afmhot_10us", extent=et, vmax=lr.max() * 0.13)
    xlim(0.2, 0.8)
    ylim(0.8, 0.2)
    axis("off")
    ...
    figure()
    subplot(131)
    imshow(lr, cmap=cmap_global, extent=et, vmax=lr.max() * 0.15)
    xlim(0.48, 0.525)
    ylim(0.535, 0.50)
    axis("off")
...
def lobe_gal_clean():
    ...
    nbit = 16
    psnr_clean = tf.image.psnr(
        DD[None, ..., None].astype(np.uint16),
        hr[None, ..., None].astype(np.uint16),
        max_val=2 ** (nbit) - 1,
    )
    psnr_polish = tf.image.psnr(
        sr[None, ..., None].astype(np.uint16),
        hr[None, ..., None].astype(np.uint16),
        max_val=2 ** (nbit) - 1,
    )

    ssim_clean = tf.image.ssim(
        DD[None, ..., None].astype(np.uint16),
        hr[None, ..., None].astype(np.uint16),
        max_val=2 ** (nbit) - 1,
    )
    ssim_polish = tf.image.ssim(
        sr[None, ..., None].astype(np.uint16),
        hr[None, ..., None].astype(np.uint16),
        max_val=2 ** (nbit) - 1,
    )
    ...
    # Create a Rectangle patch
    rect1 = patches.Rectangle(
        (810, 910), 164, 140, linewidth=3, edgecolor="C2", facecolor="none"
    )
    rect2 = patches.Rectangle(
        (a, b), 100, 100, linewidth=3, edgecolor="C2", facecolor="none"
    )
    ax2.add_patch(rect1)
    ax2.add_patch(rect2)
    ...
    rect1 = patches.Rectangle(
        (810, 910), 164, 140, linewidth=3, edgecolor="grey", facecolor="none"
    )
    rect2 = patches.Rectangle(
        (a, b), 100, 100, linewidth=3, edgecolor="grey", facecolor="none"
    )
    ax3.add_patch(rect1)
    ax3.add_patch(rect2)
    # axis('off')
    ...
    # Create a Rectangle patch
    rect1 = patches.Rectangle(
        (810, 910), 164, 140, linewidth=3, edgecolor="C0", facecolor="none"
    )
    rect2 = patches.Rectangle(
        (a, b), 100, 100, linewidth=3, edgecolor="C0", facecolor="none"
    )
    ax4.add_patch(rect1)
    ax4.add_patch(rect2)
    axis("off")
...
def vla_mosaic():
    ...
    for ii in range(4):
        for jj in range(4):
            dsr = resolve_single(
                model, d[1024 * ii : 1024 * (ii + 1), 1024 * jj : 1024 * (jj + 1)]
            ).numpy()
            arr[
                2 * 1024 * ii : 2 * 1024 * (ii + 1), 2 * 1024 * jj : 2 * 1024 * (jj + 1)
            ] = dsr
...
def plot_vla_polish_highres():
    ...
    rect2 = patches.Rectangle(
        (a2, b2), c2 - a2, d2 - b2, linewidth=3, edgecolor="C1", facecolor="none"
    )
    ax1.add_patch(rect1)
    ax1.add_patch(rect2)
    axis("off")
    ...
    rect2 = patches.Rectangle(
        (a2, b2), c2 - a2, d2 - b2, linewidth=3, edgecolor="C0", facecolor="none"
    )
    ax4.add_patch(rect1)
    ax4.add_patch(rect2)
    axis("off")
...
def plot_vla_polish():
    ...
    ax2 = plt.subplot(gs[0, 4:6])
    ax2.imshow(
        sr.reshape(-1, 4, len(sr) // 4, 4).mean(1).mean(-1),
        cmap=cmap_global,
        vmax=vmx,
        vmin=vmn,
    )
...
def gather(fnhr, fnsr, fnlr=None):
    ...
    if fnlr is None:
        for ii in range(x_image_hr.shape[0]):
            r_sr = np.sqrt(
                (y_image_hr[ii] - y_image_sr) ** 2 + (y_image_hr[ii] - x_image_sr) ** 2
            )
            ind_sr = np.argmin(r_sr)
            indarr_hr.append(ind_sr)
            if r_sr.min() < 10.0:
                param_arr_all.append(
                    [
                        A_image_hr[ii],
                        B_image_hr[ii],
                        flux_auto_hr[ii],
        ...
        for ii in range(x_image_hr.shape[0]):
            ...
            if fnlr is not None:
                r_lr = np.sqrt(
                    (y_image_hr[ii] - y_image_lr) ** 2
                    + (y_image_hr[ii] - x_image_lr) ** 2
                )
...
def match(fn1, fn2, pixel_scale_arcsec=0.5):
    ...
    pixel_scale = pixel_scale_arcsec / 3600.0
...
def plot_all(
    ...
    ra1, dec1, ra2, dec2, p1, p2, idx = match(
        fn1, fn2, pixel_scale_arcsec=pixel_scale_arcsec
    )
    ...
    if fn3 is not None:
        ra1, dec1, ra3, dec3, p1, p3, idx3 = match(
            fn1, fn3, pixel_scale_arcsec=pixel_scale_arcsec
        )
        (
            number_3,
            flux_auto_3,
    ...
    subplot(121)
    scatter(
        A_image_1 * pixel_scale_arcsec,
        A_image_2[idx] * pixel_scale_arcsec,
        np.log(snr_2[idx]),
        c="#95D840FF",
        alpha=0.75,
    )
    ...
    subplot(122)
    scatter(
        B_image_1 * pixel_scale_arcsec,
        B_image_2[idx] * pixel_scale_arcsec,
        np.log(snr_2[idx]),
        c="#95D840FF",
        alpha=0.85,
    )
    ...
    if fn3 is not None:
        ...
        scatter(
            A_image_1 * pixel_scale_arcsec,
            A_image_3[idx3] * pixel_scale_arcsec,
            np.log(snr_2[idx]),
            c="C0",
            alpha=0.5,
        )
        ...
        scatter(
            B_image_1 * pixel_scale_arcsec,
            B_image_3[idx3] * pixel_scale_arcsec,
            np.log(snr_2[idx]),
            c="C0",
            alpha=0.5,
        )
